// src/components/ExperienceEducation.jsx
import React from "react";

const experiences = [
  {
    date: "Jul 2026 – Present",
    role: "Technology Mentor – AI, Robotics & Coding",
    company: "InnovaTe X",
    location: "Nagarkurnool, India · On-site",
    bullets: [
      "Deliver AI, Robotics, and Programming education to students from Grades 6–10 through project-based STEM learning.",
      "Design and conduct hands-on sessions using Python, Arduino, sensors, robotics kits, and computational thinking activities.",
      "Mentor students in problem-solving, logical thinking, and real-world technology applications while fostering ethical AI awareness.",
      "Collaborate with school faculty to implement an engaging AI and Robotics curriculum aligned with CBSE learning objectives."
    ],
    accent: true,
  },
  {
    date: "Dec 2025 – Mar 2026",
    role: "Machine Learning Intern",
    company: "Tanvika Solutions Pvt Ltd",
    location: "Kakinada, India · On-site",
    bullets: [
      "Built AI Interview Performance Analyzer end-to-end — CNN facial emotion recognition + NLP speech evaluation pipeline using PyTorch",
      "Built Car Price Prediction MLOps Pipeline — Random Forest, Docker containerization, CI/CD automation, Streamlit deployment",
      "Full pipeline ownership from raw data to deployed product — no senior ML guidance",
    ],
    accent: false,
  },
  {
    date: "Sep 2025 – Dec 2025",
    role: "Software Developer",
    company: "Swetrasoft",
    location: "Tranås, Sweden · Remote",
    bullets: [
      "iOS application development as part of a cross-functional remote team",
      "Managed tasks and sprint workflows via Jira in an Agile environment",
      "Strengthened async collaboration and communication across time zones",
    ],
    accent: false,
  },
  {
    date: "Apr 2025 – May 2025",
    role: "Data Analyst",
    company: "ULEARN",
    location: "Kakinada, India · Remote",
    bullets: [
      "Analyzed India AQI dataset using Power BI with DAX-based calculated measures",
      "Built interactive multi-view dashboards visualizing pollution trends across regions",
      "Compiled 30-slide analytical report supporting environmental and health trend analysis",
    ],
    accent: false,
  },
  {
    date: "Oct 2023 – Apr 2026",
    role: "Club Lead",
    company: "John McCarthy AI Club · Aditya Degree College",
    location: "Kakinada, India",
    bullets: [
      "Led the official AI/ML student community under the Department of Artificial Intelligence for 2.5 years",
      "Coordinated faculty-guided projects, technical workshops, and interdisciplinary initiatives",
      "Mentored junior students on ML concepts, project development, and AI career direction",
      "Promoted ethical and responsible AI practices through hands-on workshops",
    ],
    accent: false,
  },
];

const coursework = [
  "Machine Learning",
  "Deep Learning",
  "Data Structures & Algorithms",
  "Mathematics for AI",
  "Computer Vision",
  "Natural Language Processing",
  "Statistics & Probability",
  "Database Systems",
];

export default function ExperienceEducation() {
  return (
    <section id="experience" className="py-[100px] px-8">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-24 items-start">

        {/* ── LEFT: Experience ── */}
        <div>
          <div className="vertical-accent pl-6 mb-16">
            <span className="block mb-2 text-xs font-bold tracking-[0.2em] uppercase text-[#00C9A7] font-['Space_Grotesk']">
              Experience
            </span>
            <h2 className="font-['Space_Grotesk'] text-white text-3xl font-semibold">
              Professional Journey
            </h2>
          </div>

          {/* Timeline — exact same structure as working Experience.jsx */}
          <div className="relative border-l border-[#5A9ABF]/20 ml-4 md:ml-8 pl-8 md:pl-12 space-y-16">
            {experiences.map((exp, i) => (
              <div key={i} className="relative group">

                {/* Dot — exact same positioning as working Experience.jsx */}
                <div
                  className={
                    "absolute -left-[41px] md:-left-[57px] top-1 w-4 h-4 rounded-full border-4 border-[#080E1E] transition-all duration-300 " +
                    (exp.accent
                      ? "bg-[#00C9A7] group-hover:shadow-[0_0_12px_rgba(0,201,167,0.6)]"
                      : "bg-[#00597b] group-hover:bg-[#00C9A7]")
                  }
                />

                <span className="block mb-1 text-xs font-bold tracking-widest uppercase text-slate-500 font-['Space_Grotesk']">
                  {exp.date}
                </span>

                <h3 className="font-['Space_Grotesk'] text-white text-xl font-semibold">
                  {exp.role}
                </h3>

                <p className="text-[#00C9A7] text-sm font-medium mt-1 font-['Inter']">
                  {exp.company}
                </p>

                <p className="text-[#5a9abf] text-xs mt-0.5 font-['Inter'] tracking-wide">
                  {exp.location}
                </p>

                <ul className="mt-4 space-y-2">
                  {exp.bullets.map((b, j) => (
                    <li
                      key={j}
                      className="flex items-start gap-3 font-['Inter'] text-sm text-[#8fcef5] font-light leading-relaxed"
                    >
                      <span className="mt-1.5 w-1 h-1 rounded-full bg-[#00C9A7] flex-shrink-0" />
                      {b}
                    </li>
                  ))}
                </ul>

              </div>
            ))}
          </div>
        </div>

        {/* ── RIGHT: Education ── */}
        <div>
          <div className="vertical-accent pl-6 mb-16">
            <span className="block mb-2 text-xs font-bold tracking-[0.2em] uppercase text-[#00C9A7] font-['Space_Grotesk']">
              Education
            </span>
            <h2 className="font-['Space_Grotesk'] text-white text-3xl font-semibold">
              Academic Background
            </h2>
          </div>

          <div
            className="bg-[#0D2137] p-8 space-y-6 border border-[#5A9ABF]/20 hover:border-[#00C9A7]/40 transition-all duration-300"
            style={{ boxShadow: "0 0 30px rgba(0,201,167,0.05)" }}
          >
            {/* Degree */}
            <div>
              <h3 className="font-['Space_Grotesk'] text-white text-2xl font-semibold leading-tight">
                B.Sc. Artificial Intelligence
              </h3>
              <p className="text-[#00C9A7] font-medium mt-1 font-['Inter'] text-sm">
                Aditya Degree College, Kakinada
              </p>
            </div>

            {/* Stats grid */}
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[#5A9ABF]/20">
              {[
                { label: "Duration",  value: "Aug 2023 – Apr 2026", color: "text-white" },
                { label: "GPA",       value: "8.0 / 10",            color: "text-white" },
                { label: "Focus",     value: "AI / ML",             color: "text-white" },
                { label: "Status",    value: "Graduated ✓",         color: "text-[#00C9A7]" },
              ].map((item) => (
                <div key={item.label}>
                  <p className="text-xs font-bold tracking-widest uppercase text-slate-500 font-['Space_Grotesk']">
                    {item.label}
                  </p>
                  <p className={"font-semibold font-['Space_Grotesk'] mt-1 " + item.color}>
                    {item.value}
                  </p>
                </div>
              ))}
            </div>

            {/* Coursework */}
            <div className="pt-4 border-t border-[#5A9ABF]/20 space-y-3">
              <p className="text-xs font-bold tracking-widest uppercase text-slate-500 font-['Space_Grotesk']">
                Key Coursework
              </p>
              <div className="flex flex-wrap gap-2">
                {coursework.map((c) => (
                  <span
                    key={c}
                    className="px-3 py-1 text-[#8fcef5] text-xs font-medium font-['Inter'] border border-[#5A9ABF]/20 bg-[#080E1E]"
                  >
                    {c}
                  </span>
                ))}
              </div>
            </div>

            {/* Activities */}
            <div className="pt-4 border-t border-[#5A9ABF]/20 space-y-3">
              <p className="text-xs font-bold tracking-widest uppercase text-slate-500 font-['Space_Grotesk']">
                Activities
              </p>
              {[
                "Coordinator, Robo Football League 2025",
                "Led John McCarthy AI Club — official AI/ML student community under the Department of Artificial Intelligence for 2.5 years",
              ].map((a, i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="mt-2 w-1 h-1 rounded-full bg-[#00C9A7] flex-shrink-0" />
                  <p className="font-['Inter'] text-sm text-[#8fcef5] font-light leading-relaxed">
                    {a}
                  </p>
                </div>
              ))}
            </div>
          </div>
        </div>

      </div>
    </section>
  );
}